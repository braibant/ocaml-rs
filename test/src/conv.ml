...
let () = test "enum1 empty" (fun () -> (enum1_empty () = Empty))
let () = test "enum1 first 1" (fun () -> (enum1_first 1 = First 1))
let () = test "enum1 first 9999" (fun () -> (enum1_first 9999 = First 9999))
...
let () = test "enum1 second" (fun () -> (test_second "testing" = Some "testing"))

external enum1_is_empty: enum1 -> bool = "enum1_is_empty"

let () = test "enum1 is empty 0" (fun () -> (enum1_is_empty Empty = true))
let () = test "enum1 is empty 1" (fun () -> enum1_is_empty (First 1) = false)
...
let () = test "direct slice 1" (fun () -> (
  let arr = [| 1L; 2L; 3L |] in
  Util.gc ();
  direct_slice arr = 6L
))

let () = test "deep clone 1" (fun () -> (
  let a = [1; 2; 3; 4; 5] in
  Util.gc ();
  deep_clone a = a
))
